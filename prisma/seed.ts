import { estado, servicio } from './../node_modules/.prisma/client/index.d';
  ...
  for (const usuario of usuarios) {
    if (Math.random() > 0.5) {
      ...
      const rangoProveedor = await prisma.rango_monetario.create({
        data: {
          minimo: faker.number.float({ min: 1500, max: 2000, fractionDigits: 2 }),
          maximo: faker.number.float({ min: 2001, max: 10000, fractionDigits: 2 }),
        },
      });
      const proveedor = await prisma.proveedor.create({
        data: {
          usuario_id: usuario.usuario_id,
          experiencia: `${faker.number.int({ min: 1, max: 10 })} ${faker.helpers.arrayElement(['años', 'meses'])}`,
  ...
  for (let i = 0; i < 10; i++) {
    await prisma.contrato.create({
      data: {
        ...
        monto_acordado: faker.number.float({ min: 1000, max: 10000, fractionDigits: 2 }),
